...
public class PrecisionOrgHandler {
	
	static String relStr = "leaders";
	static String relListPath = "Data/Org/leaders.txt";
	...
	static String out_path = "Data/Org/PrecisionOrgRelation.out";
		...
		for(int i=0;i<sentences.size();i++) {
			...
			for(int j=0;j<relList.size();j++) {
				if(sentence.toLowerCase().contains(relList.get(j).get("key").toLowerCase()) && 
					sentence.toLowerCase().contains(relList.get(j).get("name").substring(0, relList.get(j).get("name").length()-1).toLowerCase()) && 
					sentence.toLowerCase().contains(relList.get(j).get("org").substring(0, relList.get(j).get("org").length()-1).toLowerCase())){
					...
					if(nerTags.contains("PERSON") && nerTags.contains("ORGANIZATION")) {
								...
								try {
								    ...
								    //writer.write(fs.get(i)+"\n");
						        		writer.write(tname+"\t"+person+"\t"+relList.get(j).get("key")+"\t"+org+"\t"+sentence+"\n");
								    writer.close();
						        }catch(Exception e2) {
	...
	public static void main(String args[]) {
		...
		//String[] keywords = {"leaders","ministers","supporters"};
		String[] keywords = {"companies","candidates","politicians"};
		
		for(int iter = 0;iter<keywords.length;iter++) {
			relStr = keywords[iter];
			relListPath = "Data/Org/"+relStr+".txt";
			txtPath = "Data/xml_out_all_text.txt";
			out_path = "Data/Org/"+relStr+".out";
			readRelfromTXT();
