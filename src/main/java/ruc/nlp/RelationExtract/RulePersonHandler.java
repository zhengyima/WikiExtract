...
public class RulePersonHandler {
	...
	public void readMRfromTXT() {
		ArrayList<String> pages = new ArrayList<String>();
		ArrayList<String> RelationTags = new ArrayList<String>();
		
		File file1 = new File(modRelationTagdicPath); // 创建File类对象  
        FileInputStream fis = null; // 创建FileInputStream类对象读取File  
        InputStreamReader isr = null; // 创建InputStreamReader对象接收文件流  
        BufferedReader br = null; // 创建reader缓冲区将文件流装进去  
        try {  
            ...
            constPart.put("ModRelation", RelationTags);
        } catch (Exception e) {  
            e.printStackTrace();  
        }  finally {  
            // 文件执行完毕别忘了关闭数据流  
	...
	public void readDynamicPart() {
		...
		dynamicPart.put("Person@t2", "PERSON");
		dynamicPart.put("Orgnization", "ORGANIZATION");
	}
	...
	public ArrayList<HashMap<String,String>> match(String tname,Sentence sent){
				...
				for(int j=0;j<start_points.size();j++) {
					...
					for(;q<rule.size();) {
						...
						if(wordIsTag(sent,p,rule.get(q),nerTags)) {
							...
							if(rule.get(q).equals("Person@t2")) {
								per2 += sent.word(p)+" ";
							}
							if(rule.get(q).equals("RelationTag")) {
								rlt += sent.word(p)+" ";
							}
					...
					if(!flag) {
						...
						System.out.println("match:)");
						System.out.println("person1:"+per1);
						System.out.println("person2:"+per2);
						System.out.println("relation:"+rlt);
						try {
				        		FileWriter writer=new FileWriter(outPath,true);
						    //writer.write(fs.get(i)+"\n");
				        		writer.write(tname+"\t"+per1+"\t"+rlt+"\t"+per2+"\n");
						    writer.close();
				        }catch(Exception e2) {
	...
	public static void  main(String args[]) {
		...
		RulePersonHandler rh = new RulePersonHandler("Data/RuleData/EntityCubeFactExtData/titlePattern.txt2.bk","Data/RuleData/EntityCubeFactExtData/lstRelation.txt","Data/RuleData/EntityCubeFactExtData/modRelation.txt");
		System.out.println(new Sentence("Ma Zhengyi, 12, is an advisors's , at WTO.").word(2));
		System.out.println(new Sentence("Ma Zhengyi, 12, is an advisors's , at WTO.").nerTags());
		//System.out.println(rh.rules);
		//System.out.println(new Sentence("Ma Zhengyi's dog is an advisors at WTO.").word(2));
		//rh.match(new Sentence("Ma Zhengyi,12, is an advisors's , at WTO."));
